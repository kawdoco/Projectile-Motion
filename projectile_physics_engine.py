"""
projectile_physics_engine.py
Core physics for the Projectile Motion Simulation.

OOP pillars used here:
- Abstraction   : DragModel declares *what* a drag model must do
                  (compute an acceleration) without saying *how*.
- Inheritance   : NoDrag and QuadraticDrag both extend DragModel.
- Polymorphism  : ProjectilePhysics calls self._drag_model.acceleration(...)
                  without caring which concrete drag model it was handed.
- Encapsulation : the raw trajectory samples live in a name-mangled
                  attribute and are only reached through public methods.
"""
